...
def go_post():
  ...
  params['caption'] += "\n"
  params['caption'] += "[" + str(reddit_img_id) + "]"
  params['caption'] += "\n"
  params['caption'] += ("\nvia u/" + uploader)
  params['caption'] += "\n"
  ...
  print("IMG ID: " + str(reddit_img_id))




  update_posted = 'UPDATE pics SET posted = 1 WHERE reddit_id = "'+str(reddit_img_id)+'"'
  try: #Try adding img to database, if img is already in database then do not add
    cursor.execute(update_posted)
    cnx.commit()
  except:
    print("Could not set IMG ID " + str(reddit_img_id) + "to posted = 1 in database")
